...
use plugin\crud\model\PluginCrudItem;
...
class Agent extends Controller
{
    ...
    {
        PluginCrudItem::mQuery()->layTable(function () {
            $this->title = '代理等级管理';
    ...
    public function add()
    {
        $this->max = PluginCrudItem::maxNumber() + 1;
        PluginCrudItem::mForm('form');
    }
    ...
    {
        $this->max = PluginCrudItem::maxNumber();
        PluginCrudItem::mForm('form');
    }
    ...
    protected function _form_filter(array &$vo)
    {
        if ($this->request->isGet()) {
            ...
            $vo['number'] = $vo['number'] ?? PluginCrudItem::maxNumber();
        } else {
            $count = 0;
    ...
    public function _form_result(bool $state)
    {
        if ($state) {
            ...
            foreach (PluginCrudItem::mk()->order($order)->select() as $number => $upgrade) {
